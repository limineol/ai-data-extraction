...
def discover(home=None, environ=None):
    ...
    for name, root, pattern in [
        ...
        ('pi', Path(env.get('PI_CODING_AGENT_DIR', home / '.pi/agent')) / 'sessions', '**/*.jsonl'),
        ('omp', Path(env.get('OMP_CODING_AGENT_DIR', home / '.omp/agent')) / 'sessions', '**/*.jsonl'),
        ('openclaw', Path(env.get('OPENCLAW_STATE_DIR', home / '.openclaw')) / 'agents', '*/sessions/*.jsonl'),
        ('openclaw', Path(env.get('OPENCLAW_STATE_DIR', home / '.openclaw')) / 'agents', '*/sessions/*.jsonl.deleted.*'),
        ('openclaw', Path(env.get('OPENCLAW_STATE_DIR', home / '.openclaw')) / 'agents', '*/sessions/*.jsonl.reset.*'),
    ...
    for root in [data / 'opencode', home / 'Library/Application Support/opencode',
                 Path(env.get('APPDATA', home / 'AppData/Roaming')) / 'opencode']:
    ...
    add('hermes', Path(env.get('HERMES_HOME', home / '.hermes')), 'state.db', 'sqlite')
